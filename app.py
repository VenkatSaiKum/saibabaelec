...
def get_dashboard_data():
    ...
    try:
        ...
        # Get sales stats (with optional date filter)
        sales_summary = mgr['billing'].get_sales_summary(date=date)
        total_bills, total_sales, avg_bill, max_bill = sales_summary if sales_summary else (0, 0, 0, 0)
        
        # Get payment method breakdown (with optional date filter)
        payment_breakdown = mgr['billing'].get_payment_method_breakdown(date=date)
        ...
        data = {
            ...
            'upi_sales': payment_breakdown['upi']
        }
...
def get_bills():
    ...
    try:
        ...
        date = request.args.get('date', None)
        bills = get_managers()['billing'].get_all_bills(limit=limit if not date else None, date=date)
        
        logger.info(f"DEBUG: Found {len(bills) if bills else 0} bills")
        
        if not bills:
            logger.info("DEBUG: No bills found, returning empty array")
            return jsonify([]), 200
        ...
        for bill in bills:
            ...
            logger.info(f"DEBUG: Processing bill {bill_number}: {bill_detail}")
        ...
        logger.info(f"DEBUG: Returning {len(result)} bills")
        return jsonify(result), 200
    except Exception as e:
...
def get_bill_detail(bill_number):
    ...
    try:
        ...
        return jsonify({
            ...
            'upi_amount': bill.get('upi_amount'),
...
@app.route('/api/bills/pdf')
@login_required
def generate_bills_pdf():
    """Generate PDF for all bills on a specific date"""
    try:
        ...
        bills = get_managers()['billing'].get_all_bills(date=date)
        
        if not bills:
            return jsonify({'error': 'No bills found for this date'}), 404
        ...
        # Add title
        elements.append(Paragraph("VENKATA SAI BABA ELECTRICALS", title_style))
        elements.append(Paragraph(f"Bills Report - {date}", heading_style))
        elements.append(Spacer(1, 20))
        
        # Calculate totals
        total_bills = len(bills)
        total_amount = sum(bill[2] for bill in bills)
        cash_total = 0
        upi_total = 0
        
        # Get detailed bills with payment info
        for bill in bills:
            bill_detail = get_managers()['billing'].get_bill(bill[0])
            if bill_detail:
                payment_method = bill_detail.get('payment_method', 'CASH')
                if payment_method == 'CASH':
                    cash_total += bill_detail['total_amount']
                elif payment_method == 'UPI':
                    upi_total += bill_detail['total_amount']
                elif payment_method == 'MIXED':
                    cash_total += bill_detail.get('cash_amount', 0) or 0
                    upi_total += bill_detail.get('upi_amount', 0) or 0
        ...
        summary_data = [
            ...
            ['Cash Sales:', f'Rs.{cash_total:,.2f}'],
            ['UPI Sales:', f'Rs.{upi_total:,.2f}']
        ]
        ...
        # Bills table header
        data = [['Bill #', 'Customer', 'Time', 'Amount', 'Payment']]
        
        # Add bills data
        for bill in bills:
            bill_number, customer_name, total_amount, payment_method, created_at = bill
            # Format datetime
            try:
            ...
            # Get detailed payment info for MIXED payments
            payment_display = payment_method
            if payment_method == 'MIXED':
                bill_detail = get_managers()['billing'].get_bill(bill_number)
                if bill_detail and bill_detail.get('cash_amount') and bill_detail.get('upi_amount'):
                    cash_amt = bill_detail['cash_amount']
                    upi_amt = bill_detail['upi_amount']
                    payment_display = f"MIXED\n({cash_amt:.0f}+{upi_amt:.0f})"
            
            data.append([
                bill_number[-10:],  # Last 10 chars of bill number
                customer_name[:20],  # Truncate long names
                time_str,
        ...
        elements.append(Paragraph(f"Generated on {datetime.now().strftime('%Y-%m-%d %I:%M %p')}", footer_style))
        ...
        return send_file(
            buffer,
            as_attachment=True,
            download_name=f'bills_report_{date}.pdf',
