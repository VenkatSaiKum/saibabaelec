...
class BillingManager:
    ...
    def display_bill(self, bill_number):
        ...
        if bill['payment_method'] == 'MIXED' and bill.get('cash_amount') and bill.get('upi_amount'):
            print(f"  {'Cash Amount':<48} ₹{bill['cash_amount']:.2f}")
            print(f"  {'UPI Amount':<48} ₹{bill['upi_amount']:.2f}")
        print("="*70 + "\n")

    def get_all_bills(self, limit=None, date=None):
        """Get recent bills, optionally filtered by date"""
        if date:
    ...
    def get_sales_summary(self, date=None):
        ...
        if date:
            query = '''
                ...
                FROM transactions
                WHERE is_credit = 0 AND is_replacement = 0 AND DATE(created_at) = ?
            '''
    ...
    def get_payment_method_breakdown(self, date=None):
        """Get sales breakdown by payment method - excludes credit and replacement transactions"""
        if date:
            query = '''
                SELECT 
                    SUM(CASE WHEN payment_method = 'CASH' THEN total_amount ELSE 0 END) as cash_sales,
                    SUM(CASE WHEN payment_method = 'UPI' THEN total_amount ELSE 0 END) as upi_sales,
                    SUM(CASE WHEN payment_method = 'MIXED' THEN total_amount ELSE 0 END) as mixed_sales,
                    SUM(CASE WHEN payment_method = 'MIXED' THEN cash_amount ELSE 0 END) as mixed_cash,
                    SUM(CASE WHEN payment_method = 'MIXED' THEN upi_amount ELSE 0 END) as mixed_upi
                FROM transactions
                WHERE is_credit = 0 AND is_replacement = 0 AND DATE(created_at) = ?
            '''
        ...
        if result:
            cash_sales, upi_sales, mixed_sales, mixed_cash, mixed_upi = result
            # Combine cash from CASH and MIXED payments
            total_cash = (cash_sales or 0) + (mixed_cash or 0)
            # Combine UPI from UPI and MIXED payments
            total_upi = (upi_sales or 0) + (mixed_upi or 0)
            return {
                'cash': float(total_cash),
                'upi': float(total_upi),
                'total': float(total_cash + total_upi)
            }
        return {'cash': 0.0, 'upi': 0.0, 'total': 0.0}
